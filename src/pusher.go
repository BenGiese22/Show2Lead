...
func main() {
        now := time.Now().In(time.FixedZone("CDT", -18000))
        subtracted_time := now.Add(-10*time.Minute)
        //log.Println("Now: " + now.String() +  " .... " + "Sub: " + subtracted_time.String())

        str_now := now.Format("2006-01-02 15:04:05")
        sub_now := subtracted_time.Format("2006-01-02 15:04:05")

        prospects := GetProspectDetails(sub_now,str_now)
        for _, val := range prospects.Response.Data {
            if (val.ShowingWasScheduled == "t") {
              log.Println("Lead Email: " + val.Email + "  Agent: " + val.TeamMember)
...
func send(address string, name string, email string, phone string, agent string) {
  //TODO when change to pammke email, need to enable access to less secure apps.
  agent_name := getLeadSimpleName(agent)
  from := "bengiese22@gmail.com"
  pass := "Toddwodd1"
  to := "bengiese22@gmail.com"
